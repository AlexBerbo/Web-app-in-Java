...
<section id="about" class="about-section">
	...
	<div class="about-div">
		<h2>${poruka}</h2>
		<h2>NASE IGRAONICE</h2>
		<p>Igraonica: <%=igraonica.getNaziv()%>, <%=igraonica.getAdresa()%>, <%=igraonica.getTelefon()%></p>
		<h2>Paketi koje mozete rezervisati:</h2>
		<% for(Paket pom:pakets){
			%>
			<hr>
			<h1><%=pom.getNaziv() %></h1>
			<p><%=pom.getOpis() %>, <%=pom.getCena() %> $, <%=pom.getBodovi() %> bodova.</p>
			<a href="Rezervisi?id=<%=pom.getIdpaket() %>">Detaljnije</a>
		<% } %>
