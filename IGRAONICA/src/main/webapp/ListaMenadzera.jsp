...
<div class="about-div">
	<% ArrayList<Manager> managers=(ArrayList<Manager>)request.getAttribute("Menadzeri"); %>
</div>
	...
	<div class="about-div">
		...
		<table>
			...
			<% for(Manager pom:managers){
				%>
				<tr>
					<td><%=pom.getIdmanager() %></td>
					<td><%=pom.getIme() %></td>
					<td><%=pom.getPrezime() %></td>
					<td><%=pom.getIdigraonica() %></td>
					<td><a href="AdminObrisiMenadzera?id=<%=pom.getIdmanager() %>">Obrisi</a></td>
				</tr>
			<% } %>
		</table>
	</div>	
